...
function Login() {
  ...
  return (
    <section className="relative w-full h-full bg-white px-3 sm:px-4 md:px-6 lg:px-20 py-16 md:p-8 grid grid-cols-1 md:grid-cols-2 gap-16 md:gap-8">
      ...
      <Link to="/" className='absolute lg:fixed top-2 left-2 md:left-2 cursor-pointer'>
        <ArrowLeftStartOnRectangleIcon className='w-6 h-6 text-blackclr'/>
      </Link>

      {/* sign up sub heading */}
      <div className='relative z-10'>
        <p className='text-blackclr text-lg md:text-xl font-medium font-dmsans'>Unlock Your Access with Every Login</p>
        <h2 className="text-blackclr text-3xl md:text-4xl font-bold font-lato capitalize mt-4 mb-8 xxsm:max-w-[100%] max-w-[90%] sm:max-w-[60%] md:max-w-[80%]">Log In, Dive into the Adventure, <span className='text-buttonBottom'>&</span> Become a Member of the Exclusive Club!</h2>
      ...
      <div className='grid grid-cols-1 gap-8 relative z-10 bg-signupBg shadow-md p-8 rounded-lg'>
        <div>
          <h3 className='text-2xl text-blackclr font-bold font-lato mb-2'>Welcome Back<span className='text-buttonBottom'>!</span></h3>
          <p className='text-textGrey text-sm md:text-base font-light'>Are you eager to return to the thrill? Rejoin our dynamic community and continue your journey towards success!</p>
        </div>
        ...
        <div className='flex justify-center gap-2'>
          <h3>New Here?</h3>
          <Link to="/signup" className='text-buttonBottom'>Sign Up</Link>
        </div>
      </div>
    </section>
  )
